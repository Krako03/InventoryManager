...
class DataBaseConnectionTest {
    ...
    static void beforeAll() throws SQLException {
        dbConnection = new DBConnection(
                ...
                "sa",
                "");
        dataBaseConnection = new DataBaseConnection(dbConnection);

        try {
            dataBaseConnection.execute("CREATE TABLE providers ("
                    + "id SERIAL PRIMARY KEY, "
                    + "name VARCHAR(255) NOT NULL, "
                    + "contact VARCHAR(255) NOT NULL"
                    + ");");

            dataBaseConnection.execute("CREATE TABLE purchases ("
                    + "id SERIAL PRIMARY KEY, "
            ...
            dataBaseConnection.execute("CREATE TABLE assets ("
                    + "id SERIAL PRIMARY KEY, "
                    + "name VARCHAR(255) NOT NULL, "
                    + "description TEXT, "
            ...
            dataBaseConnection.execute("CREATE TABLE purchase_details ("
                    + "id SERIAL PRIMARY KEY, "
            ...
            dataBaseConnection.execute("CREATE TABLE employees ("
                    + "id SERIAL PRIMARY KEY, "
                    + "name VARCHAR(255) NOT NULL, "
                    + "mail VARCHAR(255) UNIQUE NOT NULL"
                    + ");");

            dataBaseConnection.execute("CREATE TABLE users ("
                    + "id SERIAL PRIMARY KEY, "
            ...
            dataBaseConnection.execute("CREATE TABLE assets_movements ("
                    + "id SERIAL PRIMARY KEY, "
            ...
            dataBaseConnection.execute("CREATE TABLE computers ("
                    + "id SERIAL PRIMARY KEY, "
            ...
            dataBaseConnection.execute("INSERT INTO providers (name, contact) VALUES "
                    + "('TechSupply', 'techsupply@example.com'), "
                    + "('OfficeWorld', 'contact@officeworld.com');");

            dataBaseConnection.execute("INSERT INTO purchases (provider_id, date, total_amount) VALUES "
                    + "(1, '2025-03-01', 2000.00), "
                    + "(2, '2025-03-02', 1500.00);");

            dataBaseConnection.execute("INSERT INTO assets (name, description, series_number) VALUES "
                    + "('Laptop', 'Dell XPS 13', 'SN-001'), "
                    + "('Desktop PC', 'HP Envy', 'SN-002'), "
                    + "('Monitor', 'Samsung 27 inch', 'SN-003'), "
                    + "('Printer', 'HP LaserJet', 'SN-004');");

            dataBaseConnection.execute("INSERT INTO employees (name, mail) VALUES "
                    + "('John Doe', 'john@example.com'), "
                    + "('Jane Smith', 'jane@example.com');");

            dataBaseConnection.execute("INSERT INTO users (employee_id, role, username, password) VALUES "
                    + "(1, 'Admin', 'john_doe', 'hashed_password_123'), "
                    + "(2, 'Employee', 'jane_smith', 'hashed_password_abc');");

            dataBaseConnection.execute("INSERT INTO purchase_details (purchase_id, asset_id, amount, price_per_item) VALUES "
                    + "(1, 1, 2, 800.00), "
                    + "(1, 3, 1, 400.00), "
                    + "(2, 2, 1, 1000.00), "
                    + "(2, 4, 1, 500.00);");

            dataBaseConnection.execute("INSERT INTO assets_movements (employee_id, asset_id, movement_type, asset_movement_date) VALUES "
                    + "(1, 1, 'ASSIGN', '2025-03-05'), "
                    + "(2, 2, 'ASSIGN', '2025-03-06');");

            dataBaseConnection.execute("INSERT INTO computers (asset_id, ram, disk, core, screen_state, keyboard_state, shell_state, comments) VALUES "
    ...
    void testProvidersPositiveInsertion() {
        try {
            dataBaseConnection.execute("INSERT INTO providers (name, contact) VALUES (?, ?)", "NewProvider", "newprovider@example.com");
    ...
    void testProviderNegativeInsertion() {
        assertThrows(SQLException.class, () -> dataBaseConnection.execute("INSERT INTO providers (name, contact) VALUES (?, ?)", "noname"));
    }

    @Order(3)
    @Test
    void testComputersNegativeInvalidAsset() {
        assertThrows(SQLException.class, () -> dataBaseConnection.execute(
